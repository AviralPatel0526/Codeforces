...
public class A_X_Axis {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t != 0) {
            ...
            int x2=in.nextInt();
            int x3=in.nextInt();
            int min1=Math.abs(x2-x1)+Math.abs(x3-x1);
            int min2=Math.abs(x1-x2)+Math.abs(x3-x2);
            int min3=Math.abs(x3-x1)+Math.abs(x3-x2);
            int ans=Math.min(min3,Math.min(min1,min2));
            out.println(ans);
        }
        out.close();
    }
