...
public class D_Flipping_and_Bonus {
    // GCD Method
    ...
    public static long f(int i,long cnt,long a[],HashMap<Long,Long> map,long dp[][]){
        if(i == a.length) return 0;
        if(dp[i][(int)cnt] != -1) return dp[i][(int)cnt];
        long heads=a[i]+(map.containsKey(cnt) ? map.get(cnt) : 0)+f(i+1,cnt+1,a,map,dp);
        long tails=f(i+1,1,a,map,dp);
        return dp[i][(int)cnt]=Math.max(heads,tails);
    }
    public static void main(String[] args) throws java.lang.Exception {
        ...
        System.out.println(f(0,1,a,map,dp));
    }
}