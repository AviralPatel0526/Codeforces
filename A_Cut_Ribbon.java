...
public class A_Cut_Ribbon {
    ...
    static int f(int n,int a,int b,int c,int dp[]){
        if(n < 0) return -1000000;
        if(n == 0) return 0;
        if(dp[n] != -1) return dp[n];
        int picka=1+f(n-a,a,b,c,dp);
        int pickb=1+f(n-b,a,b,c,dp);
        int pickc=1+f(n-c,a,b,c,dp);
        return dp[n]=Math.max(picka,Math.max(pickb,pickc));
    }
