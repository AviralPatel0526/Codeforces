...
public class C_Letter {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        // int i=0;
        // while(i<n && isc(s.charAt(i))){
        //     i++;
        //     cc--;
        // }
        // int j=n-1;
        // while( j>=0 && !isc(s.charAt(j))){
        //     j--;
        //     cs--;
        // }
        int ansc=Integer.MAX_VALUE;
        for(int i=0;i<n;i++){
            ansc=Math.min(ansc,(i+1-caps[i]+cc-caps[i]));
        }
        int anss=Integer.MAX_VALUE;
        for(int i=n-1;i>=0;i--){
            anss=Math.min(anss,((n-i)-small[i]+cs-small[i]));
        }
        System.out.println(Math.min(anss,ansc));
    }
}