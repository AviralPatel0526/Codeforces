...
public class D_Cooking {
    ...
    static void f(int i,int sum,int total,int a[],boolean dp[][]){
        if(i == a.length){
            ans=Math.min(ans,Math.max(sum,total-sum));
            return;
        }
        if(dp[i][sum]) return;
        f(i+1,sum,total,a,dp);
        f(i+1,sum+a[i],total,a,dp);
        dp[i][sum]=true;
    }
