    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t != 0) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (int j = 0; j < k; j++) { // Changed 'i++' to 'j++'
                    temp.add(in.nextLong());
                    max = Math.max(max, temp.get(j) - j); // Corrected the loop index
                }
                list.add(temp);
                first.add(new Pair(i, max));
            }
            Collections.sort(first, (p1, p2) -> Long.compare(p1.x, p2.x));
            long ans = first.get(0).x;
            long sum = list.get(first.get(0).idx).size();
            for (int i = 1; i < first.size(); i++) {
                Pair curr = first.get(i);
                ans = Math.max(ans, curr.x - sum);
                sum += list.get(curr.idx).size(); // Use 'curr.idx' for clarity
            }
