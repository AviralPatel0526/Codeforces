...
import java.lang.*;
import java.io.*;
public class C_1111_gal_password {
    ...
    static int f(int i, int prev, int n,int dp[][]) {
        if (i == n) return 1; 
        if(dp[i][prev+1] != -1) return dp[i][prev+1];
        int ans = 0;
        if (prev == -1) { 
            for (int j = 1; j <= 9; j++) {
                ans = (ans+f(i + 1, j, n,dp))%mod;
            }
        } else {
            ans =(ans +  f(i + 1, prev, n,dp))%mod;
            if (prev + 1 <= 9) {
                ans =(ans +  f(i + 1, prev + 1, n,dp))%mod;
            }
            if (prev - 1 >= 1) {
                ans = (ans + f(i + 1, prev - 1, n,dp))%mod;
            }
