...
public class B_Kevin_and_Geometry {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        if (idx == -1) {
            ...
            ArrayList<Pair> diff = new ArrayList<>();
            for (int i = 1; i < list.size(); i++) { 
                int d=list.get(i)-list.get(i-1);
                diff.add(new Pair(d, i));
            }
            Collections.sort(diff, (p1, p2) -> p1.x - p2.x);
            ...
            for (int i = 0; i < diff.size(); i++) {
                if (diff.get(i).x < max) {
                    ans.add(list.get(diff.get(i).y)); 
                    ans.add(list.get(diff.get(i).y - 1)); 
                    flag = true;
                    break;
                }
            }
