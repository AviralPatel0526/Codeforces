...
public class D_Robot_Arms_2 {
    ...
    static final int OFFSET = 10000; // Offset to handle negative indices
    static final int MAX_SUM = 20000; // Max possible sum range after shifting

    static int f(int i, int sum, int target, ArrayList<Integer> list, Integer dp[][]) {
        if (i >= list.size()) {
            return target == sum ? 1 : 0;
        }
        if (dp[i][sum + OFFSET] != null) return dp[i][sum + OFFSET];

        if (f(i + 1, sum - list.get(i), target, list, dp) == 1) {
            return dp[i][sum + OFFSET] = 1;
        }
        if (f(i + 1, sum + list.get(i), target, list, dp) == 1) {
            return dp[i][sum + OFFSET] = 1;
        }

        return dp[i][sum + OFFSET] = 0;
    }
    ...
    public static void main(String[] args) {
        ...
        Integer[][] dp1 = new Integer[xlist.size()][MAX_SUM + 1];
        Integer[][] dp2 = new Integer[ylist.size()][MAX_SUM + 1];

        boolean xPossible = f(1, xlist.get(0), x, xlist, dp1) == 1;
        boolean yPossible = f(0, 0, y, ylist, dp2) == 1;

        System.out.println(xPossible && yPossible ? "Yes" : "No");
    }
}