...
public class B_Matrix_Stabilization {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t != 0) {
            ...
            for(int i=0;i<n;i++){
                for(int j=0;j<m;j++){
                    int bot=i+1 < n ? a[i+1][j] : 0;
                    int top=i-1 >= 0 ?a[i-1][j] : 0;
                    int left=j-1 >= 0 ? a[i][j-1] : 0;
                    int right=j+1 < m  ? a[i][j+1] : 0;
                    int max=Math.max(Math.max(top,bot),Math.max(left,right));
                    int k=a[i][j];
                    if(k > bot && k > top && k > left && k > right){
                        a[i][j]=max;
                    }
                }
            }
