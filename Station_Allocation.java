...
import java.lang.*;
import java.io.*;
public class Station_Allocation {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t != 0) {
            ...
            for(int i=0;i<q;i++){
                ...
                if(lidx != -1){
                    long need=x-a[lidx];
                    long csum=sum-a[lidx];
                    need+=Math.max(0,y-csum);
                    ans=Math.min(ans,need);
                }
                if(ridx != -1){
                    long csum=sum-a[ridx];
                    long need=Math.max(0,y-csum);
                    ans=Math.min(ans,need);
                }
