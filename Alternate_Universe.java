...
public class Alternate_Universe {
    ...
    static boolean check(long a[], long b[], long mid, int n) {
        long max = 0;
        for (int i = 0; i < n; i++) {
            if (mid <= a[i]) {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t != 0) {
            t--;
            int n = in.nextInt();
            long a[] = al(n); // Read array `a` using your custom `al()` method
            long b[] = al(n); // Read array `b` using your custom `al()` method
            long l = 0;
            long h = 10000000000L;
            long ans = 0;
            while (l <= h) {
