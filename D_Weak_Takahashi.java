...
public class D_Weak_Takahashi {
    ...
    static int f(int i,int j,int r,int c,char ch[][],int dp[][]){
        if(i >= r || j >= c) return 0;
        if(dp[i][j] != -1) return dp[i][j];
        int d=0;
        if(i+1 < r && ch[i+1][j] == '.'){
            d=1+f(i+1,j,r,c,ch,dp);
        }
        int x=0;
        if(j+1 < c && ch[i][j+1] == '.'){
            x=1+f(i,j+1,r,c,ch,dp);
        }
        return dp[i][j]=Math.max(d,x);
    }
    public static void main(String[] args) throws java.lang.Exception {
        ...
        System.out.println(f(0,0,r,c,ch,dp)+1);
    }
}