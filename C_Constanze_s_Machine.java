...
public class C_Constanze_s_Machine {
    ...
    public static long f(int i,char prev,String s,long dp[][]){
        if(i == s.length()){
            return 1;
        }
        if(dp[i][prev-'a'] != -1) return dp[i][prev-'a'];
        long nochange=f(i+1,s.charAt(i),s,dp);
        long changeu=0;
        long changen=0;
        if(prev == 'u' && s.charAt(i) == 'u'){
            changeu=f(i+1,'w',s,dp);
        }
        if(prev == 'n' && s.charAt(i) == 'n'){
            changen=f(i+1,'m',s,dp);
        }
        return dp[i][prev-'a']=(nochange+changen+changeu)%mod;
    }
