...
import java.lang.*;
import java.io.*;
public class Ordered_Distances {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t != 0) {
            ...
            for(int i=0;i<n;i++){
                int diff=Math.abs(pivot-a[i]);
                if(pivot == a[i]){
                    idx=i;
                }
                list.add(new Pair(diff, a[i]));
            }
            Collections.sort(list,(p1,p2)-> p1.diff == p2.diff ? p1.num-p2.num : p1.diff-p2.diff);
            int nb[]=new int[n];
            for(int i=0;i<n;i++){
                nb[i]=list.get(i).num;
            }
