...
import java.lang.*;
import java.io.*;
public class D_Snuke_Panic_1_D {
    ...
    static long f(int i,int x,int[][] a,HashMap<Integer,Integer> map, Long[][] dp) {
        if(i >= 100001){
            return 0;
        }
        ...
        long same=ans+f(i+1,x,a,map,dp);
        long left=0;
        long right=0;
        if(x+1<5){
            right=ans+f(i+1,x+1,a,map,dp);
        }
        if(x-1 >= 0){
            left=ans+f(i+1,x-1,a,map,dp);
        }
        return dp[i][x]=Math.max(same,Math.max(left,right));
    }
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        System.out.println(f(0,0,a,map,dp));
    }
}