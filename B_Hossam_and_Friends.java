...
public class B_Hossam_and_Friends {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t-- > 0) {
            ...
            int m = in.nextInt();
            long[] a = new long[n + 1];
            Arrays.fill(a, n + 1); // Initialize to n + 1 (out of bounds)

            for (int i = 0; i < m; i++) {
                int x = in.nextInt();
                int y = in.nextInt();
                int minIdx = Math.min(x, y);
                a[minIdx] = Math.min(a[minIdx], Math.max(x, y));
            }

            // Propagate minimum "bad index" to the right
            for (int i = n - 1; i >= 1; i--) {
