...
public class C_Basketball_Exercise {
    ...
    static long f(int i,int prev,long a[],long b[],Long dp[][]){
        if(i == a.length) return 0;
        if(dp[i][prev+1] != null) return dp[i][prev+1];
        long froma=0,fromb=0;
        if(prev == -1 || prev == 1){
            froma=Math.max(a[i]+f(i+1,0,a,b,dp),f(i+1,1,a,b,dp));
        }else{
            fromb=Math.max(b[i]+f(i+1,1,a,b,dp),f(i+1,0,a,b,dp));
        }
        return dp[i][prev+1]=Math.max(froma,fromb);
    }
    public static void main(String[] args) throws java.lang.Exception {
        ...
        long ans=Math.max(f(0,-1,a,b,dp1),f(0,-1,b,a,dp2));
