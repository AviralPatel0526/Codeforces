...
public class D_FG_operation {
    ...
    static int f(int i,int prev,int a[],int x,int dp[][]){
        if(i == a.length){
            return prev == x ? 1 : 0;
        }
        ...
        int opg=0;
        if(prev == -1){
            opf=f(i+2,(a[0]+a[1])%10,a,x,dp);
            opg=f(i+2,(a[0]*a[1])%10,a,x,dp);
        }else{
            opf=f(i+1,(prev+a[i])%10,a,x,dp);
            opg=f(i+1,(prev*a[i])%10,a,x,dp);
        }
        return dp[i][prev+1]=(opf+opg)%mod;
    }
