...
public class C_Ball_in_Berland {
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        while (t != 0) {
            ...
            for(int i=0;i<k;i++){
                map1.put(x[i],map1.getOrDefault(x[i],0)+1);
                map2.put(y[i],map2.getOrDefault(y[i],0)+1);
            }
            long ans=0;
            for(int i=0;i<k;i++){
                map1.put(x[i],map1.get(x[i])-1);
                map2.put(y[i],map2.get(y[i])-1);
                ans+=k-i-1-map1.get(x[i]);
                ans+=-map2.get(y[i]);
                if(map1.get(x[i])==0){
                    map1.remove(x[i]);
                }
