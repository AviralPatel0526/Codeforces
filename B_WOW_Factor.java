...
public class B_WOW_Factor {
    ...
    static void slist(ArrayList<Long> list) {
        StringBuilder sb = new StringBuilder();
        for (long num : list) {
            sb.append(num).append(" ");
        }
    ...
    public static void main(String[] args) throws java.lang.Exception {
        ...
        long[] a = new long[cntw];
        for (int i = 0; i < cntw; i++) {
            a[i] = list.get(i) * (cntw - i);
        }
        long[] suf = new long[cntw];
        suf[cntw - 1] = a[cntw - 1];
        for (int i = cntw - 2; i >= 0; i--) {
            suf[i] = suf[i + 1] + a[i];
        }
        long ans=0;
